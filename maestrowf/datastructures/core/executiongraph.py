...
class ExecutionGraph(DAG, PickleInterface):
    ...
    def add_condenced_step_expand(self, CN):
        ...
        for every sub-node in CN unpack the step(dict) and extract these values:
                    ...
                    restart_limit: Upper limit on the number of restart attempts.
                    params: Iterable of tuples of step parameter names, values
        """
        ...
        LOGGER.info("Expanding  {0}".format(CN.name))

        # Now unpack and the CN into steps
    ...
    def write_status(self, path):
        ...
        status = [header]
        LOGGER.info("SUBTREE {0}".format(self.status_subtree))
